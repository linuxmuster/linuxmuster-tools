...
A broken image (missing/incomplete `.info` file) raises `IncompleteImageInfoError` when loaded, but `to_dict()` still reports it as `{'name': <group>, 'error': <message>, 'selected': False}` instead of taking the whole listing down with it. `.hash` is now a recognized extra file alongside `.torrent`/`.macct`/`.md5`.

The same `linbo` package also provides `LinboRemote` (build and run a `linbo-remote` command), `list_running_sessions()`/`attach_command()` (track its tmux sessions) and `classify_host()` (Off/Linbo/OS Linux/OS Windows/OS Unknown boot-state probing) - see `linuxmusterTools/linbo/README.md` for details.
